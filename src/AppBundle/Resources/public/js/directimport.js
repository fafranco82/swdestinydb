...
function import_one_line(line, lineNumber) {
	...
	if(options.length == 0) {
		...
		+qty_text+'<a class="card" data-code="'+options[0].code+'" href="#">'+options[0].label+' </a>';
	} else {
		var text = '<input type="hidden" name="'+lineNumber+'" value="'+options[0].code+':'+qty_int+'">'
		+qty_text+'<a class="card dropdown-toggle text-warning" data-toggle="dropdown" data-code="'+options[0].code+'" href="#">'+options[0].label+' <span class="caret"></span></a>';
		text += '<ul class="dropdown-menu">';
		$.each(options, function (index, option) {
			text += '<li><a href="#" data-code="'+option.code+'">'+option.label+'</a></li>';
		});
		text += '</ul>';
		return text;
	}
